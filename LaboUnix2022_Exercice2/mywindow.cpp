...
void MyWindow::on_pushButtonLogin_clicked()
{
  ...
  // TO DO
  int statut; // car estPresent retourne la pos du client si il existe et 0 pas dans la table et -1 si erreur 
  statut = estPresent(nom);
  //printf("retour de estPresent=%d\n",statut);
  if(nouveauClient)
  {
    if(statut>=1)
    {
      setResultat("« Client déjà existant ! »");
    }
    else
    {
  ...
  else
  {
    ...
    {
      if(verifieMotDePasse(statut,motDePasse))
      {
...
void MyWindow::on_pushButtonAfficheFichier_clicked()
{
  ...
  CLIENT client[100]; /// vecteur le + grand possible
  int i, nbrclient=0;

  nbrclient=listeClients(client); // je recupere le nombre de client dans le fichier  

  videTableClients();

  for(i=0;i<nbrclient;i++)
  {
    ajouteTupleTableClients(client[i].nom,client[i].hash); 
  }
